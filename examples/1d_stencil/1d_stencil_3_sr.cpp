...
// This example builds on example four. Uses S/R replacing futures from that
...
struct stepper
{
    ...
    space do_work(std::size_t np, std::size_t nt)
    {
        ...
        for (std::size_t t = 0; t != nt; ++t)
        {
            ...
            {
                auto c1 =
                    hpx::execution::experimental::just(current[idx(i, -1, np)]);
                auto c2 = hpx::execution::experimental::just(current[i]);
                auto c3 =
                    hpx::execution::experimental::just(current[idx(i, +1, np)]);
                next[i] = hpx::get<0>(hpx::this_thread::experimental::sync_wait(
                    hpx::execution::experimental::when_all(c1, c2, c3) |
                    hpx::execution::experimental::then(
                        [&](auto a, auto b, auto c) {
                            return stepper::heat_part(a, b, c);
                        })).value());
            }
        }
