...
struct stepper
{
    ...
    void fill(std::size_t np, std::size_t nx)
    {
        U.resize(2);

        // U[t][i] is the state of position i at time t.
        for (space& s : U)
            s.resize(np, partition_data(np));
    ...
    // time steps
    space do_work(std::size_t np, std::size_t nt)
    {
        ...
        for (std::size_t t = 0; t != nt; ++t)
        {
            ...
            hpx::for_each(hpx::execution::par, iterator(0), iterator(np),
                [&next, &current, np](std::size_t i) {
                    auto c1 = hpx::execution::experimental::just(
                        current[idx(i - 1, np)]);
                    auto c2 = hpx::execution::experimental::just(current[i]);
                    auto c3 = hpx::execution::experimental::just(
                        current[idx(i + 1, np)]);
                    next[i] =
                        hpx::get<0>(hpx::this_thread::experimental::sync_wait(
                            hpx::execution::experimental::when_all(c1, c2, c3) |
                            hpx::execution::experimental::then(heat_part))
                                        .value());
                });
        }
...
int hpx_main(hpx::program_options::variables_map& vm)
{
    ...
    stepper::space solution = step.do_work(np, nt);
